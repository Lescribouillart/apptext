(function (global) {
    ...
    function generateLocalSuggestions(text) {
        ...
        const { keywords, style, theme, intent } = context;
        const textType = detectTextType(text);
        const primary = keywords[0] || theme || 'inspiration';
        const secondary = keywords[1] || 'détail';
        const third = keywords[2] || 'moment';

        const phraseBase = [
            `L’${String(primary).replace(/_/g, ' ')} se révèle surtout dans les détails que personne n’attendait.`,
            `Le silence autour de ${String(primary).replace(/_/g, ' ')} disait plus que n’importe quel mot.`,
            `À mesure que ${String(primary).replace(/_/g, ' ')} avançait, la scène changeait de rythme.`,
            `Ce qui semblait banal dans ${String(primary).replace(/_/g, ' ')} finissait par devenir le cœur du récit.`,
            `La vraie émotion de ${String(primary).replace(/_/g, ' ')} ne venait pas du spectacle, mais du regard.`,
            `Il fallait laisser ${String(primary).replace(/_/g, ' ')} respirer avant de dire le reste.`,
            `Dès que ${String(secondary).replace(/_/g, ' ')} apparut, ${String(primary).replace(/_/g, ' ')} prit une tournure plus nette.`,
            `Le moment le plus fort, c’était quand ${String(primary).replace(/_/g, ' ')} devenait impossible à ignorer.`,
            `On sentait que ${String(primary).replace(/_/g, ' ')} allait faire basculer la scène.`,
            `La phrase gagnait en force quand ${String(third).replace(/_/g, ' ')} devenait plus clair.`,
            `Le lecteur comprenait tout à coup pourquoi ${String(primary).replace(/_/g, ' ')} avait été si important.`,
            `Le texte devenait plus vivant dès que ${String(primary).replace(/_/g, ' ')} gardait une trace de vérité.`
        ];

        const intentSpecific = {
            personnel: [
                `Je me suis rendu compte que ${String(primary).replace(/_/g, ' ')} avait changé quelque chose en moi.`,
                `Dans ce souvenir, ${String(primary).replace(/_/g, ' ')} n’était plus qu’une image, c’était une vérité.`,
                `Ce que je ressentais autour de ${String(primary).replace(/_/g, ' ')} était plus fort que les mots.`,
                `L’émotion venait surtout du silence qui entourait ${String(primary).replace(/_/g, ' ')}.`
            ],
            explicatif: [
                `Il fallait expliquer ${String(primary).replace(/_/g, ' ')} avec un exemple concret, pour que le sens soit clair.`,
                `Le vrai point était de montrer pourquoi ${String(primary).replace(/_/g, ' ')} changeait la situation.`,
                `Une explication simple rendait ${String(primary).replace(/_/g, ' ')} plus vivante et plus crédible.`
            ],
            persuasif: [
                `Il fallait aller plus loin avec ${String(primary).replace(/_/g, ' ')} pour convaincre le lecteur.`,
                `Le gain était clair : ${String(primary).replace(/_/g, ' ')} rendait l’idée plus forte et plus précise.`,
                `Une preuve concrète suffisait à faire passer ${String(primary).replace(/_/g, ' ')} du simple constat à l’argument fort.`
            ],
            narratif: [
                `Le tournant arrivait quand ${String(primary).replace(/_/g, ' ')} devenait impossible à ignorer.`,
                `L’action changea de direction dès que ${String(secondary).replace(/_/g, ' ')} réapparut dans la scène.`,
                `Le personnage ne comprit pas tout de suite, mais ${String(primary).replace(/_/g, ' ')} avait déjà changé la suite.`
            ],
            descriptif: [
                `La lumière autour de ${String(primary).replace(/_/g, ' ')} rendait immédiatement la scène plus vivante.`,
                `Chaque détail sur ${String(primary).replace(/_/g, ' ')} donnait au lecteur une image plus claire.`,
                `Le son, la matière et la couleur faisaient de ${String(primary).replace(/_/g, ' ')} un vrai centre de tension.`
            ]
        };

        const byStyle = {
            narratif: [
                `Le personnage continua, mais ${String(primary).replace(/_/g, ' ')} n’avait plus la même valeur qu’au départ.`,
                `Il fallait que ${String(primary).replace(/_/g, ' ')} change la direction du récit, pas seulement la scène.`,
                `La suite devenait plus intéressante dès que ${String(secondary).replace(/_/g, ' ')} prenait du poids.`
            ],
            descriptif: [
                `Le décor autour de ${String(primary).replace(/_/g, ' ')} devenait plus dense, plus concret, plus vivant.`,
                `Un seul détail sur ${String(primary).replace(/_/g, ' ')} suffisait à rendre toute la scène plus crédible.`,
                `La couleur, le bruit et la texture transformaient ${String(primary).replace(/_/g, ' ')} en véritable image.`
            ],
            analytique: [
                `Il fallait préciser comment ${String(primary).replace(/_/g, ' ')} influençait la logique de l’idée.`,
                `Le point central était que ${String(primary).replace(/_/g, ' ')} changeait la conclusion, pas seulement le décor.`,
                `Une analyse plus claire de ${String(primary).replace(/_/g, ' ')} rendait le texte plus fort.`
            ],
            persuasif: [
                `Le lecteur se laissait convaincre dès que ${String(primary).replace(/_/g, ' ')} était présenté avec un exemple concret.`,
                `La force de l’idée venait surtout de la manière dont ${String(primary).replace(/_/g, ' ')} touchait le réel.`,
                `Une phrase plus directe sur ${String(primary).replace(/_/g, ' ')} faisait immédiatement monter l’intérêt.`
            ],
            poetique: [
                `Le mot ${String(primary).replace(/_/g, ' ')} résonnait davantage quand le rythme du vers le portait.`,
                `Une image douce autour de ${String(primary).replace(/_/g, ' ')} rendait toute la phrase plus fidèle au sentiment.`,
                `Le silence autour de ${String(primary).replace(/_/g, ' ')} était devenu l’élément le plus fort de la scène.`
            ],
            intime: [
                `Je sentais que ${String(primary).replace(/_/g, ' ')} méritait une vérité plus honnête.`,
                `Le cœur savait déjà ce que la phrase devait dire à propos de ${String(primary).replace(/_/g, ' ')}.`,
                `La sensation la plus juste venait de la manière dont ${String(primary).replace(/_/g, ' ')} occupait le silence.`
            ]
        };
        ...
        const suggestions = [
            ...
            ...(textType && textType.type === 'dialogue' ? [`« ${capitalizeFirst(String(primary).replace(/_/g, ' '))} », dit-il, comme si la phrase avait déjà trouvé sa vérité.`] : []),
            ...(textType && textType.type === 'poeme' ? [`Le vers autour de ${String(primary).replace(/_/g, ' ')} gardait encore une lumière cachée.`] : []),
            ...(textType && textType.type === 'liste' ? [`On pouvait ajouter un élément utile autour de ${String(primary).replace(/_/g, ' ')} pour rendre la liste plus riche.`] : [])
        ];
        ...
        return {
            ...
            suggestions: [...new Set(suggestions.map((entry) => String(entry).trim()).filter(Boolean))].slice(0, 12)
        };
    }
